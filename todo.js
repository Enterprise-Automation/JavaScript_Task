module.exports = {create_csv, fileExists, add_username, add_sector, add_task, add_duedate, add_priority, create_record, input_id, file_check}
...
async function filter_before(cols_to_filter){
    ...
    let csv_json = await to_json();
    let date_filter_value = new Date(my_filter_bfr.bfr);
    date_filter_value = date_filter_value.toISOString().slice(0, 10);
    date_filter_value = date_filter_value.split("-").reverse().join("-");
    console.log(date_filter_value)
    let new_filtered_data = [];

    function filter(date_filter_value){
        new_filtered_data = csv_json.filter(item => {
        var compdate = item[cols_to_filter];
        return compdate < date_filter_value
        });
    }
    filter(date_filter_value);
